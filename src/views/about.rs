...
pub fn view(wid: iced::window::Id) -> Element<'static, Msg> {
    ...
    let title_col = column![
        text("Nightwave Plaza").size(14).font(bold_italic).center().width(Fill),
        Space::with_height(4),
    ...
    let panel_body = column![
        text("Contact Information").size(12).font(bold).line_height(lh),
        Space::with_height(4),
        text("Please send any inquiries you may have to mail@plaza.one.")
            .size(12)
            .line_height(lh),
        Space::with_height(4),
        text("Join our community Discord server!").size(12).line_height(lh),
        Space::with_height(8),
        ...
        text("Want to submit music for broadcast? Please use this form.")
            ...
            .line_height(lh),
        Space::with_height(8),
        text("Mobile applications (iOS / Android)").size(12).font(bold).line_height(lh),
        ...
        row![
            text("M3U (Winamp)").size(12).line_height(lh).color(LINK_COLOR),
            Space::with_width(12),
            text("PLS (Foobar2000)").size(12).line_height(lh).color(LINK_COLOR),
        ],
        Space::with_height(8),
        text("Streams").size(12).line_height(lh),
        text("http://radio.plaza.one/mp3 (mp3 / 128kbps)").size(12).line_height(lh).color(LINK_COLOR),
        text("http://radio.plaza.one/ogg (opus / 96kbps)").size(12).line_height(lh).color(LINK_COLOR),
        text("http://radio.plaza.one/hls (hls / aac)").size(12).line_height(lh).color(LINK_COLOR),
    ]
    .spacing(0);

    let panel = d3_sunken(
        container(panel_body).style(theme::panel).width(Fill).padding(8),
    );

    let pad = Padding::from([4, 24]);
    let credits_btn = raised_btn("Credits", Msg::OpenWin(crate::state::WinType::Credits), iced::Length::Shrink, pad);
    let news_btn = raised_btn("News", Msg::OpenWin(crate::state::WinType::News), iced::Length::Shrink, pad);
    ...
    let status = status_bar(vec![
        text(format!("Version: {}", version)).size(10).width(Fill).into(),
    ]);
